...
public class FluidPatternCounterPart extends AbstractMonitorPart implements IGridTickable {
    ...
    private void updatePatternCount(IGrid grid) {
        ...
        try {
            ...
            // Count patterns that use fluids (either in inputs or outputs)
            // We need to check both item and fluid craftables
            var craftableItems = craftingService.getCraftables(AEItemKey.filter());
            var craftableFluids = craftingService.getCraftables(AEFluidKey.filter());

            // Check patterns for items that involve fluids
            for (var craftableItem : craftableItems) {
                var patterns = craftingService.getCraftingFor(craftableItem);
                for (var pattern : patterns) {
                    if (hasFluidInPattern(pattern)) {
                        count++;
                    }
                }
            }

            // Check patterns for fluids
            for (var craftableFluid : craftableFluids) {
                var patterns = craftingService.getCraftingFor(craftableFluid);
                // All patterns producing fluids are fluid patterns
                count += patterns.size();
            }
    ...
    /**
     * Check if a pattern has fluid inputs or outputs.
     */
    private boolean hasFluidInPattern(IPatternDetails pattern) {
        // Check outputs for fluids
        ...
        // Check inputs for fluids
        for (var input : pattern.getInputs()) {
            for (var possibleInput : input.getPossibleInputs()) {
                if (possibleInput != null && possibleInput.what() instanceof AEFluidKey) {
                    return true;
                }
            }
        }
